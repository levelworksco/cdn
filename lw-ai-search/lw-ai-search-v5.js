...
class LwAiSearch extends LitElement {
  ...
  // ── CONFIG ──────────────────────────────────────────────────────────
  static SEARCH_BASE               = 'https://discoverai.levelworks.co';
  static SEARCH_KEY                = 'pk_live_15T99mivIvwgGZR6uyEHF906TWezsMj9SLinBUeGDe0';
  static SEARCH_INDEX_LOGICAL_NAME = 'All';
  ...
  static properties = {
    ...
    _postCommit:      { state: true },   // ← NEW: drives modal class swap
    _results:         { state: true },
    _loading:         { state: true },
  ...
  async _fetchResults(query, pageNum = 1, { prefetch = false } = {}) {
    ...
    try {
      const res = await fetch(
        `${LwAiSearch.SEARCH_BASE}/api/v1/search/${LwAiSearch.SEARCH_INDEX_LOGICAL_NAME}`,
        {
          method:  'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-API-KEY':    LwAiSearch.SEARCH_KEY,
          },
          body: JSON.stringify({
            query:         query,
            limit:         this._limit,
  ...
  _renderCard(item) {
    ...
    const href = item.canonicalUrl || `/blogs/${LwAiSearch.SEARCH_INDEX_LOGICAL_NAME}/${item.handle}`;
